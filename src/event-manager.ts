...
export class SessionEventManager {
    ...
    constructor(session: Session, id: string, timeout = 120000) {
    ...
    handleEvent(event: EventData) {
        logger.info(`Received event with ID: ${event.id}, message: ${event.message}`);
        this.session.send(event.message);
        // 在有效时间内进行相应处理
        this.createEventTimeoutObservable(event.id).subscribe(() => {
            logger.info(`Event with ID ${event.id} has expired.`);
